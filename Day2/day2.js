...
let realInput = `Game 1: 4 blue, 16 green, 2 red; 5 red, 11 blue, 16 green; 9 green, 11 blue; 10 blue, 6 green, 4 red
...
// const gameRules = [{ red: 12 }, { green: 13 }, { blue: 14 }];
let sum = 0;
realInput.split("\n").map((i) => {
  const indexOfFirst = i.indexOf(":");
  const gameIdIndex = i.indexOf("Game");
  const gameId = i.slice(gameIdIndex + 4, indexOfFirst);
  let gameValid = true;
  let row = i.slice(indexOfFirst + 1);
  let maxRed = 0,
    maxGreen = 0,
    maxBlue = 0;
  row.split(";").map((individualGame) => {
    individualGame.split(",").map((cube) => {
      if (cube.includes("red")) {
        let redCubeCount = cube.split(" ")[1];
        if (redCubeCount > 12) {
          gameValid = false;
        }

        if (Number(redCubeCount) > Number(maxRed)) {
          maxRed = Number(redCubeCount);
        }
      }
      if (cube.includes("blue")) {
        let blueCubeCount = cube.split(" ")[1];
        if (blueCubeCount > 14) {
          gameValid = false;
        }
        if (Number(blueCubeCount) > Number(maxBlue)) {
          maxBlue = Number(blueCubeCount);
        }
      }
      if (cube.includes("green")) {
        let greenCubeCount = cube.split(" ")[1];
        if (greenCubeCount > 13) {
          gameValid = false;
        }
        if (Number(greenCubeCount) > Number(maxGreen)) {
          maxGreen = Number(greenCubeCount);
        }
      }
    });
  });
  ...
  console.log("[total for this game ]", maxRed * maxBlue * maxGreen);
  // part 1 sum
  /*if (gameValid) {
  ...
  //part 2 sum
  let maxPowerSum = maxRed * maxBlue * maxGreen;
  sum = sum + maxPowerSum;
  console.log("[maxPowerSum]", sum);
});
