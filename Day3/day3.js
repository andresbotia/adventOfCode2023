...
const numbers = values.flatMap((line, lineIdx) => {
  let tempLine = [...line].join("").trim();
  ...
  const nums = line
    ...
    .reduce((acc, curr, idx) => {
      if (idx === 0) {
        const newCurr = [...curr, curr[1] + curr[0].length, lineIdx];
        const newIdx = tempLine.indexOf(curr[0]);
        const tempLine2 = tempLine.split("");

        for (i = newIdx; i < newIdx + curr[0].length; i++) {
          tempLine2[i] = "F";
        }

        tempLine = tempLine2.join("");
      ...
      return [...acc, [curr[0], newIdx, newIdx + curr[0].length, lineIdx]];
    }, []);
...
const parsedNumbers = numbers.map((num) => {
  ...
  const [_numValue, _from, _to, lineIdx] = num;

  for (i = num[1] - 1; i < num[2] + 1; i++) {
    idxToSearch.push(i);
  }

  const prevLine = values[lineIdx - 1] ?? [];
  const currLine = values[lineIdx];
  const nextLine = values[lineIdx + 1] ?? [];

  const special = [];

  const characters = [prevLine, currLine, nextLine].map((line, lineIdxChar) => {
    const chList = [];

    idxToSearch.forEach((idx) => {
      const char = line[idx];
      const isSpecial = char?.match(/[ `!@#$%^&*()_+\-=\[\]{};':"\\|,<>\/?~]/);

      if (isSpecial) {
        special.push([char, idx, lineIdx + (lineIdxChar - 1)]);
      }
  ...
  return [
    ...num,
    idxToSearch,
    characters.map((i) => i.join("")),
    !!valid,
    special.map((i) => JSON.stringify(i)),
  ];
});

const gearsObject = parsedNumbers.reduce((acc, curr) => {
  const temp = {
    ...acc,
  };
  ...
  temp[curr[7]].push(curr);

  return temp;
}, {});

const gears = Object.values(gearsObject).filter(
  (i) => i.length > 1 && i[0][7].length > 0
);

const gearRatio = gears.reduce(
  (acc, curr) => acc + parseInt(curr[0]) * parseInt(curr[1]),
  0
);

const validNumbers = parsedNumbers.filter((num) => num[6]).map((num) => num[0]);

const sum = validNumbers.reduce((acc, curr) => acc + parseInt(curr), 0);
