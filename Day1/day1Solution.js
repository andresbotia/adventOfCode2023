const input = `dssmtmrkonedbbhdhjbf9hq
  ...
  while (source.length > 0) {
    ...
    namedEntries.forEach(([name, digit]) => {
      if (forwardBuffer.includes(name)) {
        forwardBuffer = forwardBuffer.replace(
          name,
          digit + name.charAt(name.length - 1)
        );
      }
...
async function solveForFirstStar(input) {
  ...
  const calibrationValues = lines.map((line) =>
    parseCalibrationValueFrom(line)
  );
...
async function solveForSecondStar(input) {
  ...
  const replacedLines = lines.map((line) => replaceNumberWordsInString(line));

  console.log("Replaced lines:", replacedLines);

  const calibrationValues = replacedLines.map((line) =>
    parseCalibrationValueFrom(line)
  );
