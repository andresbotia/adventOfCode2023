...
a1b2c3d4e5f
treb7uchet`;
let realInput = `dssmtmrkonedbbhdhjbf9hq
...
realInput.split("\n").map((i, j) => {
  ...
  let combinedDigits = firstDigit + lastDigit;
  digits.push(combinedDigits);
});
...
const partTwoReal = `dssmtmrkonedbbhdhjbf9hq
2njsevenszzsfltconesixhsflpbpd
6shgbprkpbksnfourfivemvncvg2eight
eightseventhree7lfqpnclxnnineninemgkjtqksrdone
bsxxzhkmmfcslmdhhrgf9seven665lsknmbpgj
...
partTwoReal.split("\n").map((i) => {
  let newString = i;
  numberWords.map((number, iterator) => {
    let isNumberInside = i.includes(number[iterator + 1]);
    if (isNumberInside) {
      if (i.includes("one")) {
        newString = newString.replace("one", "o" + 1 + "e");
      }
      if (i.includes("two")) {
        newString = newString.replace("two", "t" + 2 + "o");
      }
      if (i.includes("three")) {
        newString = newString.replace("three", "t" + 3 + "e");
      }

      if (i.includes("four")) {
        newString = newString.replace("four", "f" + 4 + "r");
      }

      if (i.includes("five")) {
        newString = newString.replace("five", "f" + 5 + "e");
      }

      if (i.includes("six")) {
        newString = newString.replace("six", "s" + 6 + "x");
      }

      if (i.includes("seven")) {
        newString = newString.replace("seven", "s" + 7 + "n");
      }

      if (i.includes("eight")) {
        newString = newString.replace("eight", "e" + 8 + "t");
      }

      if (i.includes("nine")) {
        newString = newString.replace("nine", "n" + 9 + "e");
      }
    }
  });
  newParsedList += newString;
  newParsedList += "\n";
});

console.log("[newParsedList]", newParsedList);
newParsedList.split("\n").map((i) => {
  let firstDigit, lastDigit;
